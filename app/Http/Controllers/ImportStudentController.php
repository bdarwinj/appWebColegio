        ...
        foreach ($sheets as $sheetIndex => $sheet) {
            ...
            foreach ($rows as $rowIndex => $row) {
                try {
                    ...
                    if ($wordCount < 2) {
                        $errorMsg = "Hoja '{$sheetName}', fila " . ($rowIndex + 2) . ": No hay suficientes palabras en 'nombre'.";
                        $errors[] = $errorMsg;
                        Log::warning($errorMsg);
                        continue;
                    }
                    ...
                    // Lógica flexible:
                    if ($wordCount == 3) {
                        // Primera palabra para apellido, las demás para nombre
                        $apellido = $words[0];
                        $nombre = implode(' ', array_slice($words, 1));
                    } else {
                        // Si hay 4 o más palabras, se usan las dos primeras para apellido y el resto para nombre.
                        $apellido = implode(' ', array_slice($words, 0, 2));
                        $nombre = implode(' ', array_slice($words, 2));
                    }
